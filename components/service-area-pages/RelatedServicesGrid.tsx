import Link from "next/link";
import type { RelatedServiceCard } from "@/lib/service-area-pages/types";

type RelatedServicesGridProps = {
  heading: string;
  items: RelatedServiceCard[];
};

export default function RelatedServicesGrid({
  heading,
  items,
}: RelatedServicesGridProps) {
  if (!items.length) return null;

  return (
    <section className="bg-[#f7f3eb] px-5 py-14 md:py-20">
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#0f766e]">
            Related services
          </p>

          <h2 className="mt-3 text-[clamp(2rem,3vw,3.5rem)] font-black leading-[1] tracking-[-0.045em] text-[#16211f]">
            {heading}
          </h2>
        </div>

        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {items.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="group rounded-[1.5rem] border border-[#d7ded9] bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-[#0f766e] hover:shadow-md"
            >
              <h3 className="text-lg font-black tracking-[-0.025em] text-[#16211f] transition group-hover:text-[#0f766e]">
                {item.title}
              </h3>

              <p className="mt-3 text-base leading-7 text-[#5f6f69]">
                {item.description}
              </p>

              <p className="mt-5 text-sm font-bold text-[#0f766e]">
                Learn more →
              </p>
            </Link>
          ))}
        </div>
      </div>
    </section>
  );
}
